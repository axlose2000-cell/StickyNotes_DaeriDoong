...
HELP_NOTE_CONTENT = """안녕하세요 대리둥절입니다.

이 앱은 윈도우즈에서 스티커메모가 정상작동하지않는 모든분들을 위하여 개발하여 무료배포 하였으며, 무단도용,상업적이용은 금지합니다.

사용설명서

1. 기본 메모 사용법
이 앱은 여러 개의 스티커 메모를 바탕화면처럼 띄워 두고 바로 적는 용도입니다.
상단 버튼에서 새 노트를 만들고, 접기, 숨기기, 삭제, 목록 열기, 붙여넣기, 전체 복사를 빠르게 처리할 수 있습니다.
메모 본문은 바로 입력되며 내용은 자동 저장됩니다.

2. 관리 기능 사용법
노트 목록 창에서는 현재 메모들을 한 번에 보고 열기, 숨기기, 삭제를 할 수 있습니다.
노트 목록 상단의 검색 칸에서 제목+본문으로 빠르게 필터할 수 있습니다.
목록에서 노트를 선택하면 우측에 이름변경 아이콘이 나타나며, 아이콘을 누르면 제목 변경 팝업이 열려 체크/엑스로 확정 또는 취소할 수 있습니다.
또는 윈도우 폴더처럼 노트를 한 번 선택한 뒤 잠시 후 같은 항목을 다시 누르거나, F2/우클릭 메뉴의 이름변경으로 수정할 수 있습니다.
각 노트 창 상단 제목도 더블클릭, F2, 우클릭 메뉴로 바로 변경할 수 있습니다.
정렬 버튼으로 가로배치, 세로배치, 바둑판배열을 실행할 수 있고, 열려있는 메모를 한 번에 다시 띄우거나 일괄로 닫을 수 있습니다.
노트 설정에서는 각 메모의 색상, 글꼴 크기, 투명도를 개별 조정할 수 있습니다.
전역 설정에서는 자동실행, 자동시작 동작, 항상 위 표시, 백업/복원을 관리합니다.
노트 상단의 표 버튼으로 표를 입력할 수 있고, 셀 추가/삭제/병합/분할 편집 후 삽입할 수 있습니다.
노트 상단의 이미지 버튼으로 그림 파일을 삽입할 수 있으며, 그림 버튼에서 간단한 스케치를 그려 노트에 바로 넣을 수 있습니다.
삽입된 표와 이미지는 본문에서 텍스트가 아닌 실제 화면 요소로 렌더링됩니다.

3. 더 편하게 쓰는 방법
짧은 할 일, 업무 메모, 임시 복사본, 자주 보는 문구를 각각 다른 노트로 나누면 관리가 편합니다.
메모를 접어 제목줄만 남겨둘 수 있어서 화면을 덜 가리며, 숨기기 기능으로 필요할 때만 다시 열 수도 있습니다.
접힌 상태에서 붙여넣기를 누르면 노트 근처에 짧은 안내만 표시되고 실제 붙여넣기는 되지 않습니다.
내용 없는 빈 메모는 닫을 때 자동 정리되므로 부담 없이 바로 만들고 바로 지워도 됩니다.
앱은 항상 마지막 상태를 저장하며, 다음 실행(윈도우 자동시작 포함)에서도 마지막 상태 그대로 복원됩니다.
설정 창에서 "윈도우 실행 시 자동 실행"을 켜면 부팅 후 자동 시작됩니다.
윈도우 종료 시 화면에 떠 있던 메모는 다음 자동 시작 때 같은 상태로 복원됩니다.
자동 실행을 끄면 시작프로그램 등록 파일은 자동으로 정리됩니다.
한글 입력을 선택한 상태에서는 입력 중 모드가 갑자기 바뀌지 않도록 유지됩니다.
입력 중 불필요한 갱신을 줄여 타이핑이 더 자연스럽게 동작합니다.

제 앱이 마음에 드셨다면, 다른 앱도 이용해 보세요!!
1. https://play.google.com/store/apps/details?id=com.milemilesmile.work_camera_v2
업무사진을 분리,사진에 업무내용 적기! 업무전용 카메라!!

2. https://play.google.com/store/apps/details?id=com.milemilesmile.myapplication
언제든지 피아노악보연습, 멍냥이피아노!!

3. https://play.google.com/store/apps/details?id=com.axlose.meow_woof_slide
킬링타임슬라이드퍼즐 멍냥이슬라이드!!

4. 과태료 걱정없는 매장전용음악 앱! My Cafe Muse!
개발중,
"""
